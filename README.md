...
지금 상태: **화면 흐름 전체가 돌아가는 프로토타입** (가짜 데이터, 로그인 없음).
...
src/
  ...
  lib/repo/mock.ts      가짜 데이터 + 메모리/localStorage 구현 (지금 사용 중)
  lib/session.tsx       임시 로그인(계정 선택). 실제 인증은 이 파일만 교체
...
- [ ] Supabase 구현 `lib/repo/supabase.ts` + 실제 로그인
