  ...
  return (
    <>
      ...
      <section className="flex flex-col gap-3 px-4">
        ...
        {user?.role === "student" && post.status === "open" && (
          <div className="card">
            <h3 className="mb-2 font-bold">{mine ? "지원 완료" : "지원하기"}</h3>
            {mine ? <p className="sub text-sm">&ldquo;{mine.message}&rdquo; · {mine.status === "pending" ? "확인 대기 중" : mine.status === "accepted" ? "수락됨" : "거절됨"}</p> : (
              <>
          ...
          <div className="card">
            ...
            <ul className="flex flex-col gap-2 text-sm">
              {apps.map((a) => { const s = users.find((u) => u.id === a.studentId) as Extract<User, { role: "student" }> | undefined; return (
                <li key={a.id} className="rounded-xl bg-[var(--line)] px-3 py-2"><b>{s?.name}</b> <span className="sub">{s?.department}</span><p className="mt-1">{a.message}</p></li>); })}
